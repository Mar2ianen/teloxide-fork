...
path = ROOT / "crates/teloxide-core/src/requests/multipart_payload.rs"
text = path.read_text()
marker = "#[cfg(test)]\nmod tests {"
impls = """impl MultipartPayload for payloads::PostStory {
...
if text.count(marker) != 1:
    raise RuntimeError("multipart test marker changed")
text = text.replace(marker, impls + marker, 1)
text = text.replace("payloads::SendPoll,", "payloads::{EditStory, PostStory, SendPoll},", 1)
text = text.replace(
...
path = ROOT / "crates/teloxide-core/src/types/poll_answer.rs"
text = path.read_text()
test = """
...
"""
head, tail = text.rsplit("\n}", 1)
path.write_text(head + test + "\n}" + tail)

path = ROOT / "crates/teloxide-core/src/serde_multipart/integration_tests.rs"
text = path.read_text()
text = text.replace(
...
"""
path.write_text(text + wire_tests)
