...
- `DispatcherBuilder::worker_error_handler` to handle abnormal worker infrastructure events: contained handler panics, worker task deaths and undeliverable updates
...
- Support for TBA 10.0
  ...
  - Add `filter_guest_message` and `filter_managed_bot` update filters to the `UpdateFilterExt` trait
  - Add live photo, managed bot, gift background, poll media, poll option change, profile audio and video quality types
  - Add TBA 9.3 through 10.0 fields to public API types [**BC**]
...
- Contain handler panics inside dispatcher workers so that queued updates are not lost, respawn dead workers and retry the dispatch once, and report abnormal worker events through `DispatcherBuilder::worker_error_handler` instead of panicking
