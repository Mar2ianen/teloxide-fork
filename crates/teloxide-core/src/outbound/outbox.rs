    ...
    E: OutboxExecutor,
{
    ...
    /// Runs until `shutdown` resolves. No worker task is spawned internally;
    /// the bounded futures are polled by this caller-owned task.
    ...
    async fn execute_claimed(
        ...
        let acquire = match &self.settings.lane {
            Some(lane) => lane.acquire_with_correlation(metadata, correlation_id),
            None => self.queue.handle().acquire_with_correlation(metadata, correlation_id),
        };
        ...
        match self.executor.execute(request).await {
            Ok(()) => {
                ...
                self.store
                    .complete(id, lease, SystemTime::now())
                    .await
                    .map_err(OutboxWorkerError::Store)
            }
            Err(OutboxExecutionError::RetryAfter { scope, duration }) => {
                permit.complete_and_await(OutboundCompletion::RetryAfter { scope, duration }).await;
                self.retry_or_fail(
                    id,
                    lease,
                    attempt,
            ...
            Err(OutboxExecutionError::Permanent) => {
                permit.complete_and_await(OutboundCompletion::Failed).await;
                self.store
                    .fail(id, lease, OutboxFailure::new(OutboxFailureKind::Permanent))
                    .await
                    .map_err(OutboxWorkerError::Store)
            }
        }
    ...
    async fn retry_or_fail(
        ...
        if attempt >= max_attempts {
            return self
                .store
                .fail(id, lease, OutboxFailure::new(OutboxFailureKind::Exhausted))
                .await
                .map_err(OutboxWorkerError::Store);
        }
        let available_at = SystemTime::now().checked_add(delay).unwrap_or_else(SystemTime::now);
        self.store
            .reschedule(id, lease, OutboxRetry { available_at, failure })
            .await
            .map_err(OutboxWorkerError::Store)
    }
...
impl OutboxStore for InMemoryOutboxStore {
    ...
    fn complete(
        ...
        Box::pin(async move {
            ...
            if !matches!(&record.status, InMemoryStatus::Claimed(current) if *current == lease) {
...
mod tests {
    ...
    async fn lease_fencing_and_expired_lease_reclaim() {
        ...
        store.complete(id, second.lease, SystemTime::UNIX_EPOCH).await.unwrap();
    }
    ...
    async fn queue_closed_preserves_durable_record() {
        ...
        let worker = OutboundOutbox::new(store.clone(), executor, queue, settings);
        worker.run_once().await.unwrap();
        assert_eq!(store.get(id).unwrap().unwrap().status, OutboxStatus::Pending);
    }
